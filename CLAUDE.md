...
- Seção nova = schema em `sanity/schemas/objects/sections.ts` + componente em `components/sections/` + entrada no `registry.ts`. Teste `tests/registry.test.ts` força a paridade. Seções disponíveis (14): `hero`, `richText`, `ctaBanner`, `stats`, `featureGrid`, `testimonials`, `logoCloud`, `faq`, `featureSplit`, `featureCarousel`, `pricingTable`, `ctaForm`, `latestPosts`, `mediaBlock`.
